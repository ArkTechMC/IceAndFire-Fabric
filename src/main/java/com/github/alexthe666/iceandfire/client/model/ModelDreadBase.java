    ...
    public void setRotationAnglesSpawn(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        if (entityIn.getAnimation() == getSpawnAnimation()) {
            if (entityIn.getAnimationTick() < 30) {
                this.flap(armRight, 0.5F, 0.5F, false, 2, -0.7F, entityIn.age, 1);
                this.flap(armLeft, 0.5F, 0.5F, true, 2, -0.7F, entityIn.age, 1);
                this.walk(armRight, 0.5F, 0.5F, true, 1, 0, entityIn.age, 1);
                this.walk(armLeft, 0.5F, 0.5F, true, 1, 0, entityIn.age, 1);
            }
        }
    }
