...
public abstract class AbstractPathJob implements Callable<Path> {
    ...
    public static BlockPos prepareStart(LivingEntity entity) {
        ...
        if (entity.isTouchingWater())
            ...
            if (dX < ONE_SIDE) pos.set(pos.getX() - 1, pos.getY(), pos.getZ());
            else if (dX > OTHER_SIDE) pos.set(pos.getX() + 1, pos.getY(), pos.getZ());

            if (dZ < ONE_SIDE) pos.set(pos.getX(), pos.getY(), pos.getZ() - 1);
            else if (dZ > OTHER_SIDE) pos.set(pos.getX(), pos.getY(), pos.getZ() + 1);
        }
    ...
    private static int computeNodeKey(BlockPos pos) {
        return ((pos.getX() & 0xFFF) << SHIFT_X_BY) | ((pos.getY() & 0xFF) << SHIFT_Y_BY) | (pos.getZ() & 0xFFF);
    }
    ...
    protected Path search() {
        ...
        while (!this.nodesOpen.isEmpty()) {
            ...
            boolean isViablePosition = this.isInRestrictedArea(currentNode.pos) && getSurfaceType(this.world, this.world.getBlockState(currentNode.pos.down()), currentNode.pos.down()) == SurfaceType.WALKABLE;
