...
public class BlockEntityDragonForgeInput extends BlockEntity {
    ...
    public BlockEntityDragonForgeInput(BlockPos pos, BlockState state) {
        super(IafBlockEntities.DRAGONFORGE_INPUT, pos, state);
    }
    ...
    private BlockState getDeactivatedState() {
        return switch (this.getDragonType()) {
            case 1 -> IafBlocks.DRAGONFORGE_ICE_INPUT.getDefaultState().with(BlockDragonForgeInput.ACTIVE, false);
            case 2 -> IafBlocks.DRAGONFORGE_LIGHTNING_INPUT.getDefaultState().with(BlockDragonForgeInput.ACTIVE, false);
            default -> IafBlocks.DRAGONFORGE_FIRE_INPUT.getDefaultState().with(BlockDragonForgeInput.ACTIVE, false);
        };
    }
