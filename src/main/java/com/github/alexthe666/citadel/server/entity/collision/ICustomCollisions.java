...
public interface ICustomCollisions {
    ...
    static Vec3d getAllowedMovementForEntity(Entity entity, Vec3d vecIN) {
        ...
        List<VoxelShape> list = entity.method_48926().getEntityCollisions(entity, aabb.stretch(vecIN));
        Vec3d vec3 = vecIN.lengthSquared() == 0.0D ? vecIN : collideBoundingBox2(entity, vecIN, aabb, entity.method_48926(), list);
        ...
        if (entity.getStepHeight() > 0.0F && flag3 && (flag || flag2)) {
            Vec3d vec31 = collideBoundingBox2(entity, new Vec3d(vecIN.x, entity.getStepHeight(), vecIN.z), aabb, entity.method_48926(), list);
            Vec3d vec32 = collideBoundingBox2(entity, new Vec3d(0.0D, entity.getStepHeight(), 0.0D), aabb.stretch(vecIN.x, 0.0D, vecIN.z), entity.method_48926(), list);
            if (vec32.y < (double)entity.getStepHeight()) {
                Vec3d vec33 = collideBoundingBox2(entity, new Vec3d(vecIN.x, 0.0D, vecIN.z), aabb.offset(vec32), entity.method_48926(), list).add(vec32);
            ...
            if (vec31.horizontalLengthSquared() > vec3.horizontalLengthSquared()) {
                return vec31.add(collideBoundingBox2(entity, new Vec3d(0.0D, -vec31.y + vecIN.y, 0.0D), aabb.offset(vec31), entity.method_48926(), list));
            }
        }
    ...
    //1.18 logic
    private static Vec3d collideBoundingBox2(@Nullable Entity p_198895_, Vec3d p_198896_, Box p_198897_, World p_198898_, List<VoxelShape> p_198899_) {
