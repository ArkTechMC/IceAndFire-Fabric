    ...
    public void setAngles(EntityDreadGhoul thrall, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        if (thrall.getAnimation() == EntityDreadGhoul.ANIMATION_SPAWN) {
            if (thrall.getAnimationTick() < 30) {
                this.swing(armRight, 0.5F, 0.5F, false, 2, -0.7F, thrall.age, 1);
                this.swing(armLeft, 0.5F, 0.5F, true, 2, -0.7F, thrall.age, 1);
                this.flap(armRight, 0.5F, 0.5F, true, 1, 0, thrall.age, 1);
                this.flap(armLeft, 0.5F, 0.5F, true, 1, 0, thrall.age, 1);
            }
        }
        this.flap(armLeft, speed_idle, 0.15F, false, 2, -0.1F, thrall.age, 1);
        this.flap(armRight, speed_idle, 0.15F, true, 2, -0.1F, thrall.age, 1);
        this.walk(head, speed_idle, 0.1F, true, 1, -0.05F, thrall.age, 1);
