...
public class BlockEntityDragonForge extends LockableContainerBlockEntity implements SidedInventory {
    ...
    public Block getGrillBlock() {
        return switch (this.getPropertyDelegate().fireType) {
            ...
            default -> IafBlocks.DRAGONFORGE_FIRE_BRICK; // isFire == 0
        };
    }

    public boolean grillMatches(Block block) {
        return switch (this.getPropertyDelegate().fireType) {
            case 0 -> block == IafBlocks.DRAGONFORGE_FIRE_BRICK;
            case 1 -> block == IafBlocks.DRAGONFORGE_ICE_BRICK;
            case 2 -> block == IafBlocks.DRAGONFORGE_LIGHTNING_BRICK;
            default -> false;
        };
    }
    ...
    private boolean checkBoneCorners(BlockPos pos) {
        return this.doesBlockEqual(pos.north().east(), IafBlocks.DRAGON_BONE_BLOCK)
                && this.doesBlockEqual(pos.north().west(), IafBlocks.DRAGON_BONE_BLOCK)
                && this.doesBlockEqual(pos.south().east(), IafBlocks.DRAGON_BONE_BLOCK)
                && this.doesBlockEqual(pos.south().west(), IafBlocks.DRAGON_BONE_BLOCK);
    }
