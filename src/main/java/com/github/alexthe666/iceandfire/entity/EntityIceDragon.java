...
/*    @Override
    ...
    public boolean tryAttack(@NotNull Entity entityIn) {
        ...
        if (!this.isPlayingAttackAnimation()) {
            switch (groundAttack) {
                ...
                case SHAKE_PREY:
                    boolean flag = false;
                    if (new Random().nextInt(2) == 0 && isDirectPathBetweenPoints(this, this.getPos().add(0, this.getHeight() / 2, 0), entityIn.getPos().add(0, entityIn.getHeight() / 2, 0)) &&
    ...
    public void tickMovement() {
        ...
        if (!getWorld().isClient && this.isInLava() && this.isAllowedToTriggerFlight() && !this.isModelDead()) {
        ...
        if (!getWorld().isClient && attackTarget != null) {
            ...
            if (this.groundAttack == IafDragonAttacks.Ground.FIRE && (usingGroundAttack || this.isOnGround())) {
                shootIceAtMob(attackTarget);
            }
            if (this.airAttack == IafDragonAttacks.Air.TACKLE && !usingGroundAttack && this.squaredDistanceTo(attackTarget) < 100) {
        ...
        if (this.isSwimming() && !this.isModelDead()) {
            ticksSwiming++;
            if (this.isTouchingWater() && (ticksSwiming > 4000 || this.getTarget() != null && this.isTouchingWater() != this.getTarget().isTouchingWater()) && !this.isBaby() && !this.isHovering() && !this.isFlying()) {
                this.setHovering(true);
                this.jump();
        ...
        if (!getWorld().isClient && this.getControllingPassenger() == null && (this.isHovering() && !this.isFlying() && this.isTouchingWater())) {
        ...
        if (this.getRandom().nextInt(5) == 0 && !this.isBaby()) {
            if (this.getAnimation() != ANIMATION_FIRECHARGE) {
                ...
                EntityDragonIceCharge entitylargefireball = new EntityDragonIceCharge(
                    IafEntityRegistry.ICE_DRAGON_CHARGE.get(), getWorld(), this, d2, d3, d4);
            ...
            if (this.isBreathingFire()) {
                if (this.isActuallyBreathingFire()) {
                    ...
                    if (mop != null) {
                        stimulateFire(mop.getPos().x, mop.getPos().y, mop.getPos().z, 1);
                    }
                }
            } else {
...
//                    this.setDeltaMovement(this.getDeltaMovement().add(0.0D, -0.005D, 0.0D));
                ...
                if (isGoingUp() && !isGoingDown()) {
                    ...
                    vertical = -1f;
                } else if (isGoingUp() && isGoingDown() && isLogicalSideForUpdatingMovement()) {
                    // Try floating
    ...
    public void stimulateFire(double burnX, double burnY, double burnZ, int syncType) {
        ...
        for (int i = 0; i < conqueredDistance; i += increment) {
            ...
            if (canPositionBeSeen(progressX, progressY, progressZ)) {
                if (getWorld().isClient && random.nextInt(particleCount) == 0) {
                ...
                if (!getWorld().isClient) {
                    ...
                    IafDragonDestructionManager.destroyAreaBreath(getWorld(), pos, this);
                }
            }
        }
        if (burnProgress >= 40D && canPositionBeSeen(burnX, burnY, burnZ)) {
            ...
            if (!getWorld().isClient) {
                IafDragonDestructionManager.destroyAreaBreath(getWorld(), BlockPos.ofFloored(spawnX, spawnY, spawnZ), this);
            }
        }
    }
    ...
    protected void spawnBabyParticles() {
        if (this.getWorld().isClient) {
            for (int i = 0; i < 5; i++) {
                ...
                float headPosX = (float) (getX() + 1.8F * getRenderSize() * (0.3F + radiusAdd) * MathHelper.cos((float) ((getYaw() + 90) * Math.PI / 180)));
                float headPosZ = (float) (getZ() + 1.8F * getRenderSize() * (0.3F + radiusAdd) * MathHelper.sin((float) ((getYaw() + 90) * Math.PI / 180)));
