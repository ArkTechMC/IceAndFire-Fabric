...
public class WorldRenderMacros extends UiRenderMacros {
    ...
    public static void putBufferAfter(final RenderLayer bufferType, final RenderLayer putAfter) {
        ...
        if (index == -1) buffers.add(bufferType);
        else buffers.add(index + 1, bufferType);
        bufferSource = null;
    }

    public static Immediate getBufferSource() {
        if (bufferSource == null)
            bufferSource = VertexConsumerProvider.immediate(Util.make(new Object2ObjectLinkedOpenHashMap<>(), map -> buffers.forEach(type -> map.put(type, new BufferBuilder(type.getExpectedBufferSize())))), Tessellator.getInstance().getBuffer());
        return bufferSource;
    }
     ...
     */
    public static void renderBlackLineBox(final Immediate buffer, final MatrixStack ps, final BlockPos posA, final BlockPos posB, final float lineWidth) {
     ...
     * @param posB The second Position
     */
    public static void renderRedGlintLineBox(final Immediate buffer, final MatrixStack ps, final BlockPos posA, final BlockPos posB, final float lineWidth) {
     ...
     * @param posB The second Position
     */
    public static void renderWhiteLineBox(final Immediate buffer, final MatrixStack ps, final BlockPos posA, final BlockPos posB, final float lineWidth) {
     ...
     * @param aabb the box
     */
    public static void renderLineAABB(final VertexConsumer buffer, final MatrixStack ps, final Box aabb, final int argbColor, final float lineWidth) {
     ...
     * @param aabb the box
     */
    public static void renderLineAABB(final VertexConsumer buffer, final MatrixStack ps, final Box aabb, final int red, final int green, final int blue, final int alpha, final float lineWidth) {
     ...
     */
    public static void renderLineBox(final VertexConsumer buffer, final MatrixStack ps, final BlockPos pos, final int argbColor, final float lineWidth) {
     ...
     * @param posB The second Position
     */
    public static void renderLineBox(final VertexConsumer buffer, final MatrixStack ps, final BlockPos posA, final BlockPos posB, final int argbColor, final float lineWidth) {
     ...
     */
    public static void renderLineBox(final VertexConsumer buffer, final MatrixStack ps, final BlockPos posA, final BlockPos posB, final int red, final int green, final int blue, final int alpha, final float lineWidth) {
        renderLineBox(buffer, ps, Math.min(posA.getX(), posB.getX()), Math.min(posA.getY(), posB.getY()), Math.min(posA.getZ(), posB.getZ()), Math.max(posA.getX(), posB.getX()) + 1, Math.max(posA.getY(), posB.getY()) + 1, Math.max(posA.getZ(), posB.getZ()) + 1, red, green, blue, alpha, lineWidth);
    }

    /**
     * Render a box around two positions
     */
    public static void renderLineBox(final VertexConsumer buffer, final MatrixStack ps, float minX, float minY, float minZ, float maxX, float maxY, float maxZ, final int red, final int green, final int blue, final int alpha, final float lineWidth) {
        if (alpha == 0) return;
    ...
    public static void populateRenderLineBox(final float minX, final float minY, final float minZ, final float minX2, final float minY2, final float minZ2, final float maxX, final float maxY, final float maxZ, final float maxX2, final float maxY2, final float maxZ2, final Matrix4f m, final VertexConsumer buf) {
        // z plane
    ...
    public static void renderBox(final Immediate buffer, final MatrixStack ps, final BlockPos posA, final BlockPos posB, final int argbColor) {
    ...
    public static void renderBox(final VertexConsumer buffer, final MatrixStack ps, final BlockPos posA, final BlockPos posB, final int red, final int green, final int blue, final int alpha) {
        if (alpha == 0) return;
    ...
    public static void populateCuboid(final float minX, final float minY, final float minZ, final float maxX, final float maxY, final float maxZ, final Matrix4f m, final VertexConsumer buf) {
        // z plane
    ...
    public static void renderFillRectangle(final Immediate buffer, final MatrixStack ps, final int x, final int y, final int z, final int w, final int h, final int argbColor) {
        populateRectangle(x, y, z, w, h, (argbColor >> 16) & 0xff, (argbColor >> 8) & 0xff, argbColor & 0xff, (argbColor >> 24) & 0xff, buffer.getBuffer(COLORED_TRIANGLES_NC_ND), ps.peek().getPositionMatrix());
    }

    public static void populateRectangle(final int x, final int y, final int z, final int w, final int h, final int red, final int green, final int blue, final int alpha, final VertexConsumer buffer, final Matrix4f m) {
        if (alpha == 0) return;
    ...
    public static void renderDebugText(final BlockPos pos, final List<String> text, final MatrixStack matrixStack, final boolean forceWhite, final int mergeEveryXListElements, final VertexConsumerProvider buffer) {
        ...
        if (cap > 0 && erm.getSquaredDistanceToCamera(pos.getX(), pos.getY(), pos.getZ()) <= MAX_DEBUG_TEXT_RENDER_DIST_SQUARED) {
            ...
            for (int i = 0; i < cap; i += mergeEveryXListElements) {
                final MutableText renderText = Text.literal(mergeEveryXListElements == 1 ? text.get(i) : text.subList(i, Math.min(i + mergeEveryXListElements, cap)).toString());
                final float textCenterShift = (float) (-fontrenderer.getWidth(renderText) / 2);

                fontrenderer.draw(renderText, textCenterShift, 0, forceWhite ? 0xffffffff : 0x20ffffff, false, rawPosMatrix, buffer, TextRenderer.TextLayerType.SEE_THROUGH, alphaMask, 0x00f000f0);
                if (!forceWhite)
    ...
    private static final class RenderTypes extends RenderLayer {
        private static final RenderLayer GLINT_LINES = of("structurize_glint_lines", VertexFormats.POSITION_COLOR, VertexFormat.DrawMode.DEBUG_LINES, 1 << 12, false, false, MultiPhaseParameters.builder()
                .texture(NO_TEXTURE)
                ...
                .build(false));
        private static final RenderLayer GLINT_LINES_WITH_WIDTH = of("structurize_glint_lines_with_width", VertexFormats.POSITION_COLOR, VertexFormat.DrawMode.TRIANGLES, 1 << 13, false, false, MultiPhaseParameters.builder()
                .texture(NO_TEXTURE)
                .program(COLOR_PROGRAM)
                .transparency(GLINT_TRANSPARENCY)
                ...
                .build(false));
        private static final RenderLayer LINES = of("structurize_lines", VertexFormats.POSITION_COLOR, VertexFormat.DrawMode.DEBUG_LINES, 1 << 14, false, false, MultiPhaseParameters.builder()
                .texture(NO_TEXTURE)
                .program(COLOR_PROGRAM)
                ...
                .writeMaskState(COLOR_MASK)
                .build(false));
        private static final RenderLayer LINES_WITH_WIDTH = of("structurize_lines_with_width", VertexFormats.POSITION_COLOR, VertexFormat.DrawMode.TRIANGLES, 1 << 13, false, false, MultiPhaseParameters.builder()
                .texture(NO_TEXTURE)
                .program(COLOR_PROGRAM)
                .transparency(TRANSLUCENT_TRANSPARENCY)
                .depthTest(LEQUAL_DEPTH_TEST)
                ...
                .writeMaskState(ALL_MASK)
                .build(false));
        private static final RenderLayer COLORED_TRIANGLES = of("structurize_colored_triangles", VertexFormats.POSITION_COLOR, VertexFormat.DrawMode.TRIANGLES, 1 << 13, false, false, MultiPhaseParameters.builder()
                .texture(NO_TEXTURE)
                .program(COLOR_PROGRAM)
                .transparency(TRANSLUCENT_TRANSPARENCY)
                .depthTest(LEQUAL_DEPTH_TEST)
                ...
                .writeMaskState(ALL_MASK)
                .build(false));
        private static final RenderLayer COLORED_TRIANGLES_NC_ND = of("structurize_colored_triangles_nc_nd", VertexFormats.POSITION_COLOR, VertexFormat.DrawMode.TRIANGLES, 1 << 12, false, false, MultiPhaseParameters.builder()
                .texture(NO_TEXTURE)
                .program(COLOR_PROGRAM)
                .transparency(TRANSLUCENT_TRANSPARENCY)
                .depthTest(ALWAYS_DEPTH_TEST)
        ...
        private RenderTypes(final String nameIn, final VertexFormat formatIn, final VertexFormat.DrawMode drawModeIn, final int bufferSizeIn, final boolean useDelegateIn, final boolean needsSortingIn, final Runnable setupTaskIn, final Runnable clearTaskIn) {
