...
public class BlockEntityPixieHouse extends BlockEntity {
    ...
    public BlockEntityPixieHouse(BlockPos pos, BlockState state) {
        super(IafBlockEntities.PIXIE_HOUSE, pos, state);
        this.rand = new Random();
    }

    public static int getHouseTypeFromBlock(Block block) {
        if (block == IafBlocks.PIXIE_HOUSE_MUSHROOM_RED) return 1;
        if (block == IafBlocks.PIXIE_HOUSE_MUSHROOM_BROWN) return 0;
        if (block == IafBlocks.PIXIE_HOUSE_OAK) return 3;
        if (block == IafBlocks.PIXIE_HOUSE_BIRCH) return 2;
        if (block == IafBlocks.PIXIE_HOUSE_SPRUCE) return 5;
        if (block == IafBlocks.PIXIE_HOUSE_DARK_OAK) return 4;
        else return 0;
    }
