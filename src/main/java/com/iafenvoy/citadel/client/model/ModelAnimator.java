...
import com.iafenvoy.citadel.client.model.tabula.Transform;
    ...
    private void endKeyframe(boolean stationary) {
        ...
        if (animationTick >= this.prevTempTick && animationTick < this.tempTick) {
            if (stationary) {
                ...
                float tick = (animationTick - this.prevTempTick + MinecraftClient.getInstance().getTickDelta()) / (this.tempTick - this.prevTempTick);
