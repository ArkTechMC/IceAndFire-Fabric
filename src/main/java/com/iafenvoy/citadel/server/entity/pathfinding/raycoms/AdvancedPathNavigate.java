...
import com.iafenvoy.citadel.server.entity.pathfinding.raycoms.pathjobs.AbstractPathJob;
import com.iafenvoy.citadel.server.entity.pathfinding.raycoms.pathjobs.PathJobMoveAwayFromLocation;
import com.iafenvoy.citadel.server.entity.pathfinding.raycoms.pathjobs.PathJobMoveToLocation;
import com.iafenvoy.citadel.server.entity.pathfinding.raycoms.pathjobs.PathJobRandomPos;
...
public class AdvancedPathNavigate extends AbstractAdvancedPathNavigate {
    ...
    public void tick() {
        if (this.overrideDefaultDimensions) {
            this.nodeMaker.entityBlockXSize = MathHelper.floor(this.width + 1.0F);
            this.nodeMaker.entityBlockYSize = MathHelper.floor(this.height + 1.0F);
            this.nodeMaker.entityBlockZSize = MathHelper.floor(this.width + 1.0F);
        } else {
            this.nodeMaker.entityBlockXSize = MathHelper.floor(this.ourEntity.getWidth() + 1.0F);
            this.nodeMaker.entityBlockYSize = MathHelper.floor(this.ourEntity.getHeight() + 1.0F);
        ...
        if (this.entity instanceof TameableEntity tamableAnimal && tamableAnimal.isInSittingPose()) {
            return;
        }
        if (this.entity instanceof IAdvancedPathingMob advancedPathingMob && advancedPathingMob.stopTickingPathing()) {
            return;
        }
    ...
    private boolean handleLadders(int oldIndex) {
        //  Ladder Workaround
        if (!this.isIdle()) {
            assert this.getCurrentPath() != null;
            final PathPointExtended pEx = (PathPointExtended) this.getCurrentPath().getNode(this.getCurrentPath().getCurrentNodeIndex());
            final PathPointExtended pExNext = this.getCurrentPath().getLength() > this.getCurrentPath().getCurrentNodeIndex() + 1
                    ? (PathPointExtended) this.getCurrentPath().getNode(this.getCurrentPath().getCurrentNodeIndex() + 1) : null;
        ...
        if (!this.isIdle()) {
            ...
            if (pExNext != null && pEx.x == pExNext.x && pEx.z == pExNext.z)
                pExNext = this.getCurrentPath().getLength() > this.getCurrentPath().getCurrentNodeIndex() + 2
                        ? (PathPointExtended) this.getCurrentPath().getNode(this.getCurrentPath().getCurrentNodeIndex() + 2) : null;
    ...
    private boolean handleEntityInWater(int oldIndex, final PathPointExtended pEx) {
        ...
        if (curIndex > 0 && (curIndex + 1) < this.getCurrentPath().getLength() && this.getCurrentPath().getNode(curIndex - 1).y != pEx.y)
        ...
        if (vec3d.squaredDistanceTo(new Vec3d(this.ourEntity.getX(), vec3d.y, this.ourEntity.getZ())) < 0.1 && Math.abs(this.ourEntity.getY() - vec3d.y) < 0.5) {
    ...
    protected void continueFollowingPath() {
        ...
        if (wentAhead) return;
        if (curNode >= this.currentPath.getLength() || curNode <= 1) return;
