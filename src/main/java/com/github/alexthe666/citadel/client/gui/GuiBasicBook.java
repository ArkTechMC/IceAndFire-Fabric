...
public abstract class GuiBasicBook extends Screen {
    ...
    public static void drawTabulaModelOnScreen(TabulaModel model, Identifier tex, int posX, int posY, float scale, boolean follow, double xRot, double yRot, double zRot, float mouseX, float mouseY) {
        ...
        if (follow) quaternion.mul(quaternion1);
        matrixstack.multiply(quaternion);
        if (follow) matrixstack.multiply(RotationAxis.POSITIVE_Y.rotationDegrees(180.0F + f * 40.0F));
        ...
        RenderSystem.runAsFancy(() -> {
            VertexConsumer buffer = entityVertexConsumers.getBuffer(RenderLayer.getEntityCutoutNoCull(tex));
            model.resetToDefaultPose();
            model.render(matrixstack, buffer, 15728880, OverlayTexture.DEFAULT_UV, 1.0F, 1.0F, 1.0F, 1.0F);
        });
    ...
    public void drawEntityOnScreen(DrawContext guiGraphics, VertexConsumerProvider bufferSource, int posX, int posY, float zOff, float scale, boolean follow, double xRot, double yRot, double zRot, float mouseX, float mouseY, Entity entity) {
        ...
        if (follow) {
            ...
            if (entity instanceof LivingEntity livingEntity) {
                livingEntity.bodyYaw = setY;
                livingEntity.prevBodyYaw = setY;
                livingEntity.headYaw = setY;
                livingEntity.prevHeadYaw = setY;
            }
        } else f1 = 0;
    ...
    private void addNextPreviousButtons() {
        ...
        this.buttonPreviousPage = this.addDrawableChild(new BookPageButton(this, k + 10, l + 180, false, (p_214208_1_) -> this.onSwitchPage(false), true));
        this.buttonNextPage = this.addDrawableChild(new BookPageButton(this, k + 365, l + 180, true, (p_214205_1_) -> this.onSwitchPage(true), true));
    }
    ...
    public void render(DrawContext guiGraphics, int x, int y, float partialTicks) {
        ...
        int k = (this.width - this.xSize) / 2;
        int l = (this.height - this.ySize + 128) / 2;
        Identifier bindingTexture = this.getBookBindingTexture();
        guiGraphics.drawTexturedQuad(bindingTexture, k, k + this.xSize, l, l + this.ySize, 0, 0, (float) this.xSize / this.xSize, 0, (float) this.ySize / this.ySize, bindingR, bindingG, bindingB, 255);
        Identifier pageTexture = this.getBookPageTexture();
        guiGraphics.drawTexturedQuad(pageTexture, k, k + this.xSize, l, l + this.ySize, 0, 0, (float) this.xSize / this.xSize, 0, (float) this.ySize / this.ySize, 255, 255, 255, 255);
        ...
        if (this.internalPage != null) {
            guiGraphics.getMatrices().push();
            this.renderOtherWidgets(guiGraphics);
            guiGraphics.getMatrices().pop();
        }
    ...
    private void renderOtherWidgets(DrawContext guiGraphics) {
        int k = (this.width - this.xSize) / 2;
        int l = (this.height - this.ySize + 128) / 2;
        ...
        for (TabulaRenderData tabulaRenderData : this.tabulaRenders) {
            if (tabulaRenderData.getPage() == this.currentPageCounter) {
                ...
                if (model != null && texture != null) {
                    ...
                    drawTabulaModelOnScreen(model, texture, k + tabulaRenderData.getX(), l + tabulaRenderData.getY(), 30 * scale, tabulaRenderData.isFollowCursor(), tabulaRenderData.getRotX(), tabulaRenderData.getRotY(), tabulaRenderData.getRotZ(), this.mouseX, this.mouseY);
                }
            }
        }
        for (EntityRenderData data : this.entityRenders)
            if (data.getPage() == this.currentPageCounter) {
                ...
                Entity model = this.renderedEntities.putIfAbsent(data.getEntity(), type.create(MinecraftClient.getInstance().world));
                if (model != null) {
                    ...
                    this.drawEntityOnScreen(guiGraphics, guiGraphics.getVertexConsumers(), k + data.getX(), l + data.getY(), 1050F, 30 * scale, data.isFollowCursor(), data.getRotX(), data.getRotY(), data.getRotZ(), this.mouseX, this.mouseY, model);
                }
            }
        for (RecipeData recipeData : this.recipes)
            if (recipeData.getPage() == this.currentPageCounter) {
    ...
    protected void renderRecipe(DrawContext guiGraphics, Recipe<?> recipe, RecipeData recipeData, int k, int l) {
        ...
        for (int i = 0; i < ingredients.size(); i++) {
            ...
            if (!ing.isEmpty()) {
                ...
                } else stack = ing.getMatchingStacks()[0];
            }
            if (!stack.isEmpty()) {
                guiGraphics.getMatrices().push();
    ...
    protected void writePageText(DrawContext guiGraphics) {
    ...
    protected BookPage generatePage(Identifier res) {
        ...
        try {
            ...
            if (resource.isPresent()) {
                BufferedReader reader = resource.get().getReader();
                            ...
                            if (indexes.isDown()) {
                                if (yIndex >= (buttonY) / 12F && yIndex <= (buttonY + height) / 12F)
                                    if (buttonX < 90 && xIndex < 90 || buttonX >= 90 && xIndex >= 90)
                                        yIndex += 2;
                            } else if (yIndex >= (buttonY - height) / 12F && yIndex <= (buttonY + height) / 12F)
                                if (buttonX < 90 && xIndex < 90 || buttonX >= 90 && xIndex >= 90)
                                    yIndex++;
                        }
                    }
