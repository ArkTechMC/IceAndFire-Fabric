...
public class GuiBestiary extends Screen {
    ...
    private static Item getItemByRegistryName(String registryName) {
        return ForgeRegistries.ITEMS.getValue(new Identifier(registryName));
    }
    ...
    protected void init() {
        ...
        this.addDrawableChild(previousPage);
    ...
    public void render(@NotNull DrawContext ms, int mouseX, int mouseY, float partialTicks) {
        ...
        if (!index) {
            ...
            textRenderer.draw(String.valueOf(pageLeft), (float) centerX, (float) (centerY - (Y * 0.13)), 0X303030, false, ms.getMatrices().peek().getPositionMatrix(), ms.getVertexConsumers(), TextRenderer.TextLayerType.NORMAL, 0, 15728880);
            textRenderer.draw(String.valueOf(pageRight), (float) centerX, (float) (centerY - (Y * 0.13)), 0X303030, false, ms.getMatrices().peek().getPositionMatrix(), ms.getVertexConsumers(), TextRenderer.TextLayerType.NORMAL, 0, 15728880);
        }
        ms.getMatrices().pop();
                ...
                for (String line : lines) {
                    ...
                    if (line.contains("<") || line.contains(">")) {
                        if (line.contains("<image>")) {
                            ...
                            String texture = IceAndFire.MOD_ID,"textures/gui/bestiary/" + split[0];
    ...
    public void writeFromTxt(DrawContext ms) {
        ...
        try {
            ...
            for (String line : lines) {
                ...
                if (linenumber <= 19) {
                    textRenderer.draw(line, 15, 20 + linenumber * 10, 0X303030, false, ms.getMatrices().peek().getPositionMatrix(), ms.getVertexConsumers(), TextRenderer.TextLayerType.NORMAL, 0, 15728880);
                } else {
                    textRenderer.draw(line, 220, (linenumber - 19) * 10, 0X303030, false, ms.getMatrices().peek().getPositionMatrix(), ms.getVertexConsumers(), TextRenderer.TextLayerType.NORMAL, 0, 15728880);
                }
                linenumber++;
                ms.getMatrices().pop();
            }
        ...
        float scale = textRenderer.getWidth(s) <= 100 ? 2 : textRenderer.getWidth(s) * 0.0125F;
        ms.getMatrices().scale(scale, scale, scale);
        textRenderer.draw(s, 10, 2, 0X7A756A, false, ms.getMatrices().peek().getPositionMatrix(), ms.getVertexConsumers(), TextRenderer.TextLayerType.NORMAL, 0, 15728880);
        ms.getMatrices().pop();
    }

    private boolean usingVanillaFont() {
        return textRenderer == MinecraftClient.getInstance().textRenderer;
    }
