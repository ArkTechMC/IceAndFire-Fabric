...
public class AmphithereAIFollowOwner extends Goal {
    ...
    public boolean canStart() {
        ...
        else if (LivingEntity instanceof PlayerEntity && LivingEntity.isSpectator()) return false;
        else if (this.ampithere.isSitting()) return false;
        else if (this.ampithere.squaredDistanceTo(LivingEntity) < this.minDist * this.minDist) return false;
        else {
    ...
    public boolean shouldContinue() {
        return !this.noPath() && this.ampithere.squaredDistanceTo(this.owner) > this.maxDist * this.maxDist && !this.ampithere.isSitting();
    }

    private boolean noPath() {
        if (!this.ampithere.isFlying()) return this.ampithere.getNavigation().isIdle();
        else return false;
    }
    ...
    @Override
    public void tick() {
        this.ampithere.getLookControl().lookAt(this.owner, 10.0F, this.ampithere.getMaxLookPitchChange());

        if (!this.ampithere.isSitting()) {
            if (--this.timeToRecalcPath <= 0) {
                this.timeToRecalcPath = 10;
                this.tryMoveTo();
                if (!this.ampithere.isLeashed() && !this.ampithere.hasVehicle() && this.ampithere.squaredDistanceTo(this.owner) >= 144.0D) {
                    ...
                    for (int l = 0; l <= 4; ++l)
                        for (int i1 = 0; i1 <= 4; ++i1)
                            if ((l < 1 || i1 < 1 || l > 3 || i1 > 3) && this.canTeleportToBlock(new BlockPos(i, j, k))) {
                                this.ampithere.refreshPositionAndAngles(i + l + 0.5F, k, j + i1 + 0.5F, this.ampithere.getYaw(), this.ampithere.getPitch());
                                this.ampithere.getNavigation().stop();
                                return;
                            }
                }
    ...
    private void tryMoveTo() {
        if (!this.ampithere.isFlying()) this.ampithere.getNavigation().startMovingTo(this.owner, this.followSpeed);
        else
